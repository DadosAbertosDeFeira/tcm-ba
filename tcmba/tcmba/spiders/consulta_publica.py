...
class ConsultaPublicaSpider(Spider):
    ...
    def parse(self, response):
        ...
        payload = {
            ...
            'javax.faces.partial.execute': f'{form_id}:searchButton @component',
    ...
    def get_search_results(self, response):
        ...
        result_rows = selector.xpath("//tbody[@id='consultaPublicaTabPanel:consultaPublicaDataTable:tb']//tr")
        view_state = selector.xpath("//*[@id='javax.faces.ViewState']/text()").get()
        ...
        for table_row in [result_rows[0]]:
            ...
            payload = {
                ...
                'javax.faces.partial.render': '@component',
                'org.richfaces.ajax.component': f'{form_id}:selecionarPrestacao',
                f'{form_id}:selecionarPrestacao': f'{form_id}:selecionarPrestacao',
                'rfExt': 'null',
                'AJAX:EVENTS_COUNT': '1',
                'javax.faces.partial.ajax': 'true'
            }
    ...
    def get_detailed_results(self, response):
        ...
        result_rows = selector.xpath("//tbody[@id='consultaPublicaTabPanel:tabelaDocumentos_data']//tr")
        view_state = selector.xpath("//*[@id='javax.faces.ViewState']/text()").get()
        unit = selector.xpath(
            "//span[@id='consultaPublicaTabPanel:unidadeJurisdicionadaDecoration:unidadeJurisdicionada']/text()").get()

        payloads = []

        for row in result_rows:
            columns = row.xpath("./td")
            texts = [text.strip() for text in columns.xpath(".//text()").extract() or [] if text.strip()]
            ...
            payloads.append(dict(url='https://e.tcm.ba.gov.br/epp/ConsultaPublica/listView.seam',
                                 formdata=payload,
    ...
    def prepare_file_request(self, response):
        yield Request(url="https://e.tcm.ba.gov.br/epp/PdfReadOnly/downloadDocumento.seam",
    ...
    def save_file(self, response):
        ...
        with open(f"{self.settings['FILES_STORE']}/{item['filename']}", 'wb') as fp:
    ...
    def get_selector(self, response):
        source_code = response.xpath("//partial-response//changes").get() \
            .replace("&lt;", "<").replace("&gt;", ">")
